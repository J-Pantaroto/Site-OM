...
function _buscar() {
  _buscar = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
    var pesquisa,
      escopo,
      url,
      resposta,
    ...
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        case 0:
          ...
          escopo = _args2.length > 1 && _args2[1] !== undefined ? _args2[1] : '';
          url = '';
          if (!(escopo === 'usuarios')) {
            _context2.next = 7;
            break;
        ...
        case 18:
          ...
          tabelaContainer = document.querySelector('tbody');
          pagination = document.getElementById('paginacao');
...
var searchBtn = document.getElementById('pesquisar');
if (searchBtn) {
  searchBtn.addEventListener('click', function (event) {
    event.preventDefault();
    var pesquisaInput = document.querySelector('input[name="pesquisa"]').value;
    var escopo = document.querySelector('input[name="escopo"]').value;
...
if (rotaAtual === 'configuracoes.edit') {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    stateSelect.addEventListener("change", function () {
      var ibgeCode = this.value;
      citySelect.disabled = true;
      citySelect.innerHTML = '<option value="" disabled selected>Carregando...</option>';
      fetch("/cities/".concat(ibgeCode)).then(function (response) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    var stateSelect = document.getElementById("state"); // Dropdown de estado
    var citySelect = document.getElementById("city"); // Dropdown de cidade
    var zipInput = document.getElementById("zip_code"); // Campo de CEP
    var userState = stateSelect.getAttribute("data-selected-state"); // Estado pré-selecionado
    var userCity = citySelect.getAttribute("data-selected-city"); // Cidade pré-selecionada
    ...
    function loadCities(stateAbbreviation) {
      var preselectedCity = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      citySelect.disabled = true;
      citySelect.innerHTML = '<option value="" disabled selected>Carregando...</option>';
      fetch("/cities/".concat(stateAbbreviation)) // Chamada para buscar cidades
    ...
    zipInput.addEventListener("blur", function () {
      var zipCode = zipInput.value.replace(/\D/g, ""); // Remover caracteres não numéricos
      if (zipCode.length === 8) {
        fetch("https://viacep.com.br/ws/".concat(zipCode, "/json/")).then(function (response) {
          return response.json();
        }).then(function (data) {
          if (!data.erro) {
            ...
            var stateAbbreviation = data.uf.toUpperCase();
            stateSelect.value = stateAbbreviation;
            stateSelect.dispatchEvent(new Event("change"));

            // Selecionar cidade após carregar
            setTimeout(function () {
              var cityOption = Array.from(citySelect.options).find(function (option) {
                return option.textContent === data.localidade;
              });
              if (cityOption) {
      ...
      fetch(form.action, {
        ...
        if (error.errors) {
          form.querySelectorAll("input, select, textarea").forEach(function (el) {
            return el.classList.remove("border-red-500");
          });
          Object.keys(error.errors).forEach(function (field, index) {
          ...
          sweetalert2__WEBPACK_IMPORTED_MODULE_0__.fire({
            ...
            html: Object.entries(error.errors).map(function (_ref2) {
              var _ref3 = _slicedToArray(_ref2, 2),
                field = _ref3[0],
                messages = _ref3[1];
              return "<p>".concat(field, ": ").concat(messages.join(", "), "</p>");
            }).join("")
          });
        }
      });
