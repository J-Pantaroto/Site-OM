...
if (usuarioAutenticado) {
  ...
  cartItems.addEventListener('click', function (event) {
    ...
    if (inputGrupo) {
      ...
      if (button.classList.contains('button-minus')) {
        if (value > 1) {
          ...
          document.getElementById("cart-count").textContent = contagem;
          verificarBotaoFinalizar();
          var _quantidadeTotal = 0;
          var _totalGeral = 0;
          var _cartTotal = document.querySelector('#cartTotal');
          var _cartCount = document.getElementById('cart-count');
          if (exibirPreco) {
            Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
              var quantidadeSpan = row.querySelector('.quantity-span');
              var subtotalCell = row.querySelector('.subtotal-cell');
            ...
            _cartCount.textContent = _quantidadeTotal;
            _cartTotal.textContent = "R$ ".concat(_totalGeral.toFixed(2));
          } else {
            Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
              var quantidadeSpan = row.querySelector('.quantity-span');
              var quantidade = parseInt(quantidadeSpan.textContent);
      ...
      if (button.classList.contains('button-plus')) {
        ...
        document.getElementById("cart-count").textContent = contagem;
        verificarBotaoFinalizar();
        var _quantidadeTotal2 = 0;
        var _totalGeral2 = 0;
        var _cartTotal2 = document.querySelector('#cartTotal');
        var _cartCount2 = document.getElementById('cart-count');
        if (exibirPreco) {
          Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
            var quantidadeSpan = row.querySelector('.quantity-span');
            var subtotalCell = row.querySelector('.subtotal-cell');
            var quantidade = parseInt(quantidadeSpan.textContent);
            var subtotal = parseFloat(subtotalCell.textContent.replace('R$', '').trim());
            _quantidadeTotal2 += quantidade;
            _totalGeral2 += subtotal;
          });
          _cartCount2.textContent = _quantidadeTotal2;
          _cartTotal2.textContent = "R$ ".concat(_totalGeral2.toFixed(2));
        } else {
          Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
            var quantidadeSpan = row.querySelector('.quantity-span');
            var quantidade = parseInt(quantidadeSpan.textContent);
    ...
    if (button.classList.contains('remover-item')) {
      var _item = button.closest('tr');
      var _nomeProduto = _item.querySelector('td:nth-child(2)').textContent;
      ...
      var _totalGeral3 = 0;
      var _cartTotal3 = document.querySelector('#cartTotal');
      var _cartCount3 = document.getElementById('cart-count');
      if (exibirPreco) {
        Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
          var quantidadeSpan = row.querySelector('.quantity-span');
          var subtotalCell = row.querySelector('.subtotal-cell');
          var quantidade = parseInt(quantidadeSpan.textContent);
          var subtotal = parseFloat(subtotalCell.textContent.replace('R$', '').trim());
          _quantidadeTotal3 += quantidade;
          _totalGeral3 += subtotal;
        });
        _cartCount3.textContent = _quantidadeTotal3;
        _cartTotal3.textContent = "R$ ".concat(_totalGeral3.toFixed(2));
      } else {
        Array.from(cartItems.querySelectorAll('tr')).forEach(function (row) {
          var quantidadeSpan = row.querySelector('.quantity-span');
          var quantidade = parseInt(quantidadeSpan.textContent);
...
function adjustLayout() {
  ...
  titulosAdd.forEach(function (tituloAdd) {
    tituloAdd.classList.remove('fs-6');
    if (window.innerWidth < 1000) {
      tituloAdd.classList.add('fs-6');
    }
  });
  linksAdd.forEach(function (linkAdd) {
